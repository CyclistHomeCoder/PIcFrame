...
parse.add_argument("-a", "--blur_amount",   default=8, type=float, help="larger values than 12 will increase processing load quite a bit")
...
parse.add_argument("-c", "--check_dir_tm",  default=120.0, type=float, help="time in seconds between checking if the image directory has changed")
...
parse.add_argument(      "--min_size", 	    default=5000, type=int)
parse.add_argument("-n", "--recent_n",      default=0, type=int, help="when shuffling the keep n most recent ones to play before the rest")
parse.add_argument("-o", "--font_file",     default="/home/pi/PIcFrame/fonts/NotoSans-Regular.ttf")
parse.add_argument("-p", "--pic_dir",       default="/home/pi/Pictures")
parse.add_argument("-q", "--shader",        default="/home/pi/PIcFrame/shaders/blend_new")
parse.add_argument("-r", "--reshuffle_num", default=1, type=int, help="times through before reshuffling")
parse.add_argument("-s", "--show_names_tm", default=0, type=float, help="time to show text over image with file name")
parse.add_argument(      "--skip_dir",      default="@eaDir", help="skip directories with this name")
...
parse.add_argument("-v", "--time_delay",    default=15.0, type=float, help="time between consecutive slide starts - can be changed by MQTT")
parse.add_argument("-w", "--fade_time",     default=3.0, type=float, help="change time during which slides overlap - can be changed by MQTT")
...
parse.add_argument("-z", "--blur_zoom",     default=2.0, type=float, help="must be >= 1.0 which expands the backgorund to just fill the space around the image")
